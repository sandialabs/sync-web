...
fn test_sync_let() {
    ...
    assert(
        "(catch #t\
           (lambda () (sync-let () (set! sync-state (lambda () #t)) #f) #f)\
           (lambda args #t))",
        "#t",
    );
    assert(
        "(sync-let ()\
           (set! byte-vector-length (lambda (value) 7))\
           (byte-vector-length #u()))",
        "7",
    );
    assert(
    ...
    assert(
        "(begin (sync-let () (set! + 1) #t) (= (sync-let () (+ 1 2)) 3))",
        "#t",
    );
    assert(
        "(begin (sync-let () (define ephemeral-capability 7) #t)\
                (catch #t\
                  (lambda () (sync-let () ephemeral-capability) #f)\
                  (lambda args #t)))",
        "#t",
    );
    assert(
        "(begin\
           (catch #t\
             (lambda () (sync-let () (set! list 1) (error 'expected \"failure\")))\
             (lambda args #t))\
           (equal? (sync-let () (list 1 2)) '(1 2)))",
        "#t",
    );
    assert(
        "(begin\
           (sync-let () #t)\
           (let ((symbol (string->symbol \"late-root-capability\")))\
             (eval (list 'define symbol 7) (rootlet))\
             (let ((run (lambda (body)\
                          (%sync-let-return\
                           (%sync-let '() '() (expression->byte-vector body))))))\
               (list\
                (catch #t (lambda () (run symbol) 'leaked) (lambda args 'caught))\
                (catch #t\
                  (lambda () (run (list 'set! symbol 9)) 'mutated)\
                  (lambda args 'caught))\
                (run (list 'setter (list 'quote symbol)))\
                (catch #t\
                  (lambda ()\
                    (run (list 'set!\
                               (list 'setter (list 'quote symbol))\
                               '(lambda (symbol value) value)))\
                    'mutated)\
                  (lambda args 'caught))\
                (catch #t (lambda () (run symbol) 'leaked) (lambda args 'caught))\
                (eval symbol (rootlet))))))",
        "(caught caught #f caught caught 7)",
    );
    assert(
        "(sync-let ()\
           (letrec ((value 0)\
    ...
    assert(
        "(let ((secret 9))\
           (sync-let ((expr '(lambda (node) secret)))\
             (catch #t\
               (lambda ()\
                 (if (equal?\
                      (sync-eval\
                       (sync-cons (expression->byte-vector expr) (sync-null)))\
                      9)\
                     'leaked 'isolated))\
...
#[test]
fn test_unapproved_execution_helpers_are_unavailable() {
    let (_record, assert) = setup_record();
    for name in [
        "sync-call-program",
        "%sync-call-program",
        ...
        assert(
            format!(
                "(catch #t (lambda () ({name}) #f) (lambda args #t))"
            )
            .as_str(),
            "#t",
        );
        assert(
            format!(
                "(sync-let () (catch #t (lambda () ({name}) #f) (lambda args #t)))"
            )
            .as_str(),
            "#t",
        );
