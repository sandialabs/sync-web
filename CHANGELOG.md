...
- **Gateway authentication** — Kratos session cookies validated at the gateway boundary; unauthenticated requests rejected with 401. `Authorization: Bearer <secret>` accepted for machine/automation callers. Resolved identity forwarded to all journal calls.
...
- **Compose defaults** — All service image tags default to `1.1.0`.
