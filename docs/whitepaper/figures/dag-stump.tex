\begin{tikzpicture}[
  ...
  \node[record, below=6mm of roothandle, minimum width=31mm] (rootrecord)
    {node handle $w$\\persistent flag $p$};
  \node[dag node, below=6mm of rootrecord] (rootnode) {N};
  \draw[lookup] (roothandle) -- node[right,note] {roots lookup} (rootrecord);
  \draw[lookup] (rootrecord) -- node[right,note] {referenced node} (rootnode);
  \node[panel, fit=(roothandle)(rootrecord)(rootnode),
