...
\begin{tikzpicture}[
  ...
  \node[input, above=6mm of eval.north] (args) {explicit arguments $a$};
  \node[input, below=6mm of eval.south] (effects) {prepared external values $e$};
  ...
  % Conceptual boundary note.
  \draw[swgold!85!black, dashed, rounded corners=4pt]
    ($(eval.north west)+(-3mm,8mm)$) rectangle ($(eval.south east)+(3mm,-8mm)$);
  \node[note, text=swgold!60!black, below=12mm of eval.south]
    {self-encoding does not mean self-sufficient};
\end{tikzpicture}
