...
A bridge, digest, signature, or path does not imply trust, endorsement, authorization, or semantic truth. Partial structure is not confidentiality; private final-mile communication requires access control and TLS, with additional encryption where the application demands it. Federation supplies neither anonymity, onion routing, offline private relay, nor generic exactly-once execution.

Self-encoding definitions are not guaranteed to terminate, remain compatible, or behave correctly. Discovery, moderation, governance, key recovery, human-readable naming, and legal erasure remain institutional and application concerns. The architecture can preserve evidence relevant to those decisions, but it cannot make the decisions socially legitimate.
