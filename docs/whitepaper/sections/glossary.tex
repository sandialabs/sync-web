...
\begin{description}[style=nextline]
  ...
  \item[Authorization]
    A policy decision determining whether a principal may perform an operation. In current federation, terminal-local rules grant authenticated remote principals independent path-scoped \texttt{put!}, qualified \texttt{use!}, \texttt{run!}, and \texttt{retrieve} permissions. Namespace ownership and read-only \texttt{use!} do not imply \texttt{run!}; Root and configured Interface administrators remain local principals with default authority.
  ...
  \item[Journal signing-key continuity]
    A receiver-local chain from an accepted signing key through predecessor-authorized transitions in committed heads. Journals are infrastructure rather than durable principals; bridge aliases name authorization relationships locally.
  ...
  \item[Origin-local pin]
    Retention of verified remote proof material in the originating journal under the full origin-relative historical path. It follows a federated \texttt{retrieve} and mutates the origin's retention state.
  ...
  \item[Working route]
    The ordered receiver-local directional aliases used from the origin's latest committed view to select the terminal authentication object, endpoint, and reverse path to the origin key. Its implicit indexes are current; \texttt{retrieve} may select historical content independently.
