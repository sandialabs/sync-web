...
\begin{description}[style=nextline]
  ...
  \item[Historical cursor]
    A vector containing one origin index plus one index per working-route hop, used only by \texttt{resolve}. The indexes are interpreted against the route's ordered journal names while remaining independent of its current authentication and endpoint context.
  ...
  \item[Origin-local pin]
    Retention of verified remote proof material in the originating journal under the full origin-relative bridge and history path. It follows a federated \texttt{resolve} and mutates the origin's retention state.

  \item[Path]
    A sequence interpreted from a selected journal root to address a resource or intermediate structure. Like a relative URL, its meaning is completed by a starting root.
  ...
  \item[Working route]
    The ordered receiver-local bridge names used from the origin's latest committed view to select the terminal authentication object, endpoint, and reverse path to the origin key.
