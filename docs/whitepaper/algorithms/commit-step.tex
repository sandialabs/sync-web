...
\Function{CommitStep}{$L,t,p_k,s_k$}
  ...
  \State $r \gets \Call{PendingKeyTransition}{L}$
  \State $k_0 \gets \Call{AcceptedSigningKey}{L}$
  \State $\mathit{rotate} \gets (p_k \ne k_0)$
  \If{$\mathit{rotate}$}
    \State $\Call{VerifyAuthorizedTransition}{L,r,k_0,p_k,\lvert P\rvert}$
  \ElsIf{$r$ is present}
    \State \Call{Fail}{``pending transition does not match signing key''}
  \EndIf
  ...
  \EndIf
  \If{$d_{cur}=d_{prev}$ and not $\mathit{rotate}$}
  ...
  \State $H \gets P[-1]$
  \State $H \gets \Call{EmbedHeadMetadata}{H,L.\mathit{identity},L.\mathit{name},L.\mathit{endpoint},L.\mathit{interfaceKey},r}$
  \State $C \gets \Call{BlankSigningFields}{H}$ \Comment{Generic and journal-namespaced key/signature fields.}
  \State $d_s \gets \Call{Digest}{C}$
  \State $\sigma \gets \Call{Sign}{s_k,d_s}$
  \State $P[-1] \gets \Call{FillSigningFields}{C,p_k,\sigma}$
  \If{$\mathit{rotate}$}
    \State \Call{CommitKeyTransition}{$L,r,p_k$}
  \EndIf
  \State $T.\Call{Push}{P[-1]}$
  \State $q_t \gets \Call{DeepSlice}{\operatorname{Node}(T),\texttt{(-1 (*state* *time*))}}$
  \If{$L.\mathit{window}$ is set and $\Call{Size}{T}>L.\mathit{window}$}
    \State $T.\Call{Prune}{-(L.\mathit{window}+1)}$
  \EndIf
  \State $q_p \gets \Call{DeepPrune}{\operatorname{Node}(P),\texttt{(-1 (*state*))}}$
