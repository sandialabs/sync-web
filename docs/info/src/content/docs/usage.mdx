...
| `authentication` | alist / object | Restricted calls | `((identity (self)) (credentials (<secret>)))` — identity chain and matching credentials |
...
((function get)
 (arguments ((path ((*state* docs article hash)))
             (pinned? #t)
             (proof? #t)))
 (authentication ((identity (self)) (credentials ("password")))))
```
