...
2. Browse `/ledger/-1/state` for current committed content (`-1` always refers to the current head).
3. Browse `/ledger/<index>/state` for a specific prior committed snapshot.
4. Traverse `/ledger/-1/bridge/<name>/state` for a bridged peer's current committed state.
...
│   └── ...
├── ledger/
│   └── <index>/          (-1 = current head; positive integers = historical snapshots)
│       ├── state/
│       │   └── ...
│       └── bridge/
│           └── <name>/
