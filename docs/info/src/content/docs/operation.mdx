...
```bash
SECRET=password tests/api/local-compose.sh up
```
...
| SMB share | File-system projection: `/stage` (mutable), `/ledger/state` (current committed, index optional), `/control/pin` (pin directives). |
...
((function *secret*)
 ...
 (authentication ((identity *journal*) (credentials "old-password"))))
```
