...
Status: historical planning note, superseded for federation details by `federation.md` and the current `dev-1.5` implementation. In particular, signed application federation is now limited to `get`, `set!`, and `resolve`; interface administrators are local principals; pin/unpin and all administration remain local.
