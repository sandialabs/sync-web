...
Status: exploratory follow-on note. Sync Web 1.5 implements the narrower model in `federation.md`: signed remote application calls are limited to `get`, `set!`, `get-batch`, `set-batch!`, and `resolve`; pin/unpin and administration are origin-local. The arbitrary query, materialization, and step-job concepts below are future possibilities, not current federation behavior.
