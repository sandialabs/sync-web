...
Status: the federation protocol is implemented on `dev-1.5` across the Scheme records, Gateway, Explorer, deployment configuration, and social-agent model. Signed application federation is explicitly limited to `get`, `set!`, and `resolve`. Dedicated Scheme tests cover multi-journal semantics, while the social-agent stack exercises real HTTP transport and the Explorer working/history route UI. The former publisher/subscriber and push/pull policy machinery has been removed rather than carried into federation.
...
├── invocation identity
│   ├── current private interface key
│   └── retained private keys required by committed key epochs
└── peers
    └── alias
...
(bridge! ledger alias endpoint)
(delete-bridge! ledger alias)

(synchronize! ledger request)
(bridge-synchronize! ledger alias)
(step! ledger)

(route ledger request)
(invoke ledger operation arguments route history identity)
...
Federation receives the live Ledger object as an explicit dependency. It does not maintain a parallel copy of committed peer history. On a fresh installation, Federation is the sole authority for private interface signing keys; Root never stores a parallel key copy. This release line rejects nonfresh state atomically rather than migrating legacy keys or peer state.
...
Open auto-accept mode can consume durable state. The records layer deliberately does not impose bridge-count, route-hop, or serialized-node ceilings; serialized inputs are still parsed strictly without evaluation. Comprehensive resource control is deferred to unified evaluator metering, while transport-level byte limits, request-rate limits, preapproval, and administrative cleanup remain deployment tools.
...
Function, arguments, private paths, and values never traverse intermediate journals. Missing stumps may be filled lazily at the exact committed indexes, but embedded journals are never advanced during routing. The terminal must be directly reachable for the private call.
...
10. for `resolve`, carries the separately indexed Ledger path through the same hops and checks returned content/proof against that historical terminal object before returning it.

Pinning is not sent through this signed invocation. After resolving remote content, the origin may locally pin the full origin-relative bridge/history path. The origin fetches and verifies any missing proof material through the constrained public `trace` protocol, then mutates only its own retention state.


The private key remains inside the journal/runtime trust boundary. No generic arbitrary-byte signing endpoint is exposed.
...
A non-empty originating `route-target`, or a signed terminal invocation with a non-empty `route-source`, is valid only for one of those three scalar functions. Records enforce the allowlist at both boundaries: the origin rejects a disallowed operation before route/proof/network work, and the terminal rejects it before application dispatch. Gateway checks provide earlier client errors but are not the authority.

The three functions have distinct consistency roles:
...
`get` and `set!` use only the canonical working route. They do not accept a historical cursor. Only `resolve` accepts the independently indexed Ledger history path. `set-batch!` and `batch!` are deliberately excluded; atomic or efficient remote batching can be designed later if a concrete application requires it.
...
Interface administrators are local principals only. A bridge-shaped remote principal cannot gain broad administrator bypass; remote callers always require explicit path-scoped `get`, `set!`, and/or `resolve` rules. The authorization rule shape contains only those three permissions plus principal, path, and terminal authentication `key-index`. `key-index` is required for remote principals and omitted for local/public principals. Public routing/trace policy remains structural and separate from application-owner authorization.

### Gateway and Explorer implications

The Gateway accepts non-empty `$federation.route` only for `get`, `set`, and `resolve`. `$federation.history` is valid only for `resolve`, requires a non-empty route, and contains one index for the origin plus one for each route hop. Federation context on local-only aliases, `batch`, direct protocol operations, and root operations is rejected with a clear client error rather than ignored.

Explorer's working route applies to Stage `get`/`set!` and Ledger `resolve`. Access and Admin are unavailable while a remote working route is selected, because silently administering Self beneath a remote breadcrumb would be ambiguous. Returning to Self exposes those panels. Remote Ledger pin/unpin controls issue origin-local retention calls with the full bridge/history path and without `$federation` context. Explorer does not need a direct-path escape hatch: when the selected owner root is an ancestor of an applicable grant, the interface returns each ancestor directory's ordinary immediate listing. Folder names remain navigable, while opening a listed child still requires its own direct or descendant authorization.
...
The social-agent model exercises ordinary scoped authorization rather than remote-admin bypass: positive and negative remote `get`, `set!`, and `resolve`, ancestor traversal with child-level denial, and local retention of resolved remote proofs. It creates configurable deterministic users on every journal; each user's fixed keys are split between a public remotely read-only subtree and a private subtree available only to the same username over every simple non-revisiting route within the configured segment bound. Independent per-user activity updates assigned keys in place. It does not federate pin/unpin or install bridge principals as interface administrators.

The route handshake remains general: live `get`/`set!` need only its canonical cursor, while `resolve` adds the mirrored historical cursor. Adding another federated function later is a protocol-surface decision requiring explicit semantics, authorization mapping, Gateway/Explorer behavior, negative tests, and documentation; implementing a normal local query method does not automatically federate it.
...
The old interface key does not need to sign the new one because the verified journal head authorizes the replacement. During rotation, the origin retains at most the currently committed private key and its pending replacement. Once a signed head commits the replacement, the previous private key is removed. A route that selected a retired key must be recompiled against the newer signed head rather than extending secret-key retention. Private bridge configuration cannot override the key selected by the compiled verified proof.
...
- interface private keys confined to journal runtime and pruned after pending replacement commits;
...
18. **Federated application surface:** exactly `get`, `set!`, and `resolve` may use a signed federated application invocation. Records enforce this allowlist at origin and terminal. Direct `route`/`trace`/`synchronize!` protocol calls are a separate content-authenticated/public-control category; every other query and root operation is local-only.
...
20. **Complementary route cursors:** the working-journal route uses canonical `-1` traversal for authentication and live `get`/`set!`. Ledger exposes a mirrored, independently indexed history path used only by `resolve`; journal identity comes from the stable committed journal ID rather than active-key equality when the paths diverge.
