...
Each top-level Journal session builds one sealed capability template from this fixed allowlist over a sealed rootless mask. Every `sync-let` still receives a fresh child environment with fresh mutable capability slots, copies its inputs and outputs, and is discarded after the boundary. The template ancestry has no ambient outlet, so capabilities or symbols added later cannot become visible; mutations and definitions in one child cannot affect the template or another child. The same template is used by primitive-traced serialization queries without sharing query results or application state.
