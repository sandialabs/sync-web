...
- Same-user private authorization over bounded simple non-revisiting routes, with different-user and loop denial
...
- **Activity**: Seconds between per-user activity cycles. The default is `4`; `0` disables continuous activity and is reserved for setup-only or benchmark baselines.
- **Users**: Number of deterministic non-admin fixture users (compose/runtime `USERS`).
- **Segments**: Maximum simple federated route length for same-user private authority (compose/runtime `SEGMENTS`, default `2`).
