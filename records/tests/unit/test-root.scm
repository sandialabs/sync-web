    ...
    (return
     (map test
          `(((sync-call '(,root-src "pass" #t) #t) "Installed root module")
            ...
            (((root 'copy!) '(a) '(a*)) #t)
            (((root 'get) '(a* c d)) 4)
            ;; Copying a missing source applies normal `(nothing)` deletion
            ;; semantics and never leaves a directory ghost.
            (((root 'set!) '(copy-target) 'old) #t)
            (((root 'copy!) '(missing) '(copy-target)) #t)
            (((root 'get) '(copy-target)) '(nothing))
            (((root 'get) '())
             (lambda (result)
               (and (pair? result) (eq? (car result) 'directory)
                    (not (assoc 'copy-target (cadr result))))))

            ;; Root remains a path map: scalar writes and scalar copies cannot
            ;; replace the complete directory at the empty path.
            (((root 'set!) '() 'scalar)
             (lambda (result) (and (pair? result) (eq? (car result) 'error))))
            (((root 'copy!) '(a b) '())
             (lambda (result) (and (pair? result) (eq? (car result) 'error))))
            (((root 'get) '(a b)) 2)
