(lambda (assertions-src standard-src authorization-src)
  ...
  ;; Bridge control-plane admission belongs to Interface/Federation, not policy.
  ;; Authorization itself remains policy-only for these paths.
  ...
  (assert (authorize? admins local-peer (request 'trace '(-1 *bridge* journal-0)) 10) #f)
  (assert (authorize? admins local-peer (request 'trace '(-1 *bridge* journal-0 -1 *crypto* interface public-key)) 10) #f)
  ...
  ;; Rule shape, remote key windows, and ranges fail fast during configuration.
  (assert (catch #t
                 (lambda ()
                   (authorize! owner
