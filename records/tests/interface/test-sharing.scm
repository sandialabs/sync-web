  ...
  (with-let (make-interface-harness :journals 3 :users '(alice bob carol))
    ...
    (define alice-rule
      `((principal ,alice-principal) (key-index (-10 -1))
        (path (private)) (use! ((read-only? #t))) (put! #t) (retrieve #t)))
    ...
    (test-submit ((*journal* journal-3 'put!) '(*state* bob published) "public-v1") :expect #t)
    (test-submit ((*journal* journal-3 'put!) '(*state* bob private) "private-v1") :expect #t)
    (test-submit ((*journal* journal-3 'put!) '(*state* bob hidden) "hidden-v1") :expect #t)
    (test-submit ((*journal* journal-3 'put!) '(*state* bob *directory*) #t) :expect #t)
    (test-submit
    ...
    (test-submit ((*journal* journal-2 'put!) '(*state* bob seed) "middle") :expect #t)
    (for-each
      (lambda (journal)
    ...
    (test-submit
      ((alice journal-1 journal-2 journal-3 'use!) '(*state* bob private))
      :schedule '(3 1 2 0 1 2) :expect "private-v1")
    (test-submit
      ((carol journal-1 journal-2 journal-3 'use!) '(*state* bob published))
      :schedule '(1 3 0 2 2 0) :expect "public-v1")
    (test-submit
      ((bob journal-2 journal-3 'use!) '(*state* bob published))
      :schedule '(2 1 1 0) :tick 1 :expect "public-v1")
    (test-submit ((bob journal-2 journal-3 'use!) '(*state* bob private)) :schedule '(1 2 0 1) :expect error-result?)
    (test-report)
    ...
    ;; Stage reflects the completed write immediately, while retrieve remains on
    ...
    (test-submit
      ((carol journal-1 journal-2 journal-3 'put!)
       '(*state* bob published) "forbidden")
      :schedule '(2 1 1 2 0 1) :expect error-result?)
    (test-submit ((alice journal-2 journal-3 'use!) '(*state* bob private)) :schedule '(1 2 0 1) :expect error-result?)
    ...
    (test-submit
      ((alice journal-1 journal-2 journal-3 'use!) '(*state* bob))
    ...
    (test-submit
      ((carol journal-1 journal-2 journal-3 'use!) '(*state* bob))
      ...
      :expect (lambda (result)
                (and (eq? (car result) 'directory)
                     ...
                     (not (assoc '*directory* (cadr result))))))
    (test-submit
      ((carol journal-1 journal-2 journal-3 'use!) '(*state* bob hidden))
    ...
    (test-report)
    (test-submit
      ((*journal* journal-3 'use!) '(*state* bob private) :expression? #f)
      :expect #u(0 1 2 255))
    (test-submit
      ((alice journal-1 journal-2 journal-3 'use!)
    ...
    (define carol-rule
      `((principal ,carol-principal) (key-index (-10 -1))
        (path (hidden)) (use! ((read-only? #t))) (put! #f) (retrieve #t)))
    ...
    (define alice-read-rule
      `((principal ,alice-principal) (key-index (-10 -1))
        (path (private)) (use! ((read-only? #t))) (put! #f) (retrieve #t)))
