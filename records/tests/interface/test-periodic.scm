(lambda (make-interface-harness)
  (with-let (make-interface-harness
    ...
    (test-submit
      ...
      :expect #t)
    (test-submit ((alice journal-1 'put!) periodic-path periodic-program)
      :expect error-result?)
    (test-submit
      ((alice journal-1 'put-batch!)
       `((paths (,periodic-path)) (values (,periodic-program)) (expression? #t)))
      :expect error-result?)
    (test-submit ((admin journal-1 'put!) periodic-path periodic-program)
      :expect #t)
    (test-report)
    ...
    (test-report)
    (test-submit ((*journal* journal-1 'use!) '(*state* periodic-runs 0))
      :expect 1)

    ;; Rotation updates both the stored credential and verifier. The next outer
    ;; step reenters run! with the new value and launches index 1 exactly once.
    (test-submit
    ...
    (test-submit ((*journal* journal-1 'use!) '(*state* periodic-runs 0))
      :expect 1)
    (test-submit ((*journal* journal-1 'use!) '(*state* periodic-runs 1))
      :expect 1)
    ...
    (test-report)
    (test-submit ((*journal* journal-1 'use!) '(*state* periodic-runs 2))
      :expect 1)
    (test-submit ((*journal* journal-1 'use!) '(*state* periodic-runs 3))
      :expect 1)
    ...
    (test-submit ((admin journal-1 'put!) periodic-path '(nothing))
      :expect #t)
    (test-submit ((*journal* journal-1 'step!)) :expect 5)
    (test-report)
    (test-submit ((*journal* journal-1 'use!) '(*state* periodic-runs 4))
      :expect '(nothing))
    ...
    (test-submit
      ((admin journal-1 'put!) periodic-path '(lambda (journal index) index))
      :expect #t)
