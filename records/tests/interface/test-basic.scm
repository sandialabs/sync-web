(lambda (make-interface-harness)
  ...
  ;; General batch! composes ordinary Interface requests and commits only when
  ;; every subrequest succeeds.
  (test-submit
    ((*journal* journal-1 'batch!)
     '((queries
        (((function set!)
          (arguments ((path (*state* batch gamma))
                      (value "g") (expression? #t))))
         ((function get)
          (arguments ((path (*state* batch gamma)) (expression? #t))))))))
    :expect '(#t "g"))
  (test-submit
    ((*journal* journal-1 'batch!)
     '((queries
        (((function set!)
          (arguments ((path (*state* batch rolled-back))
                      (value "temporary") (expression? #t))))
         ((function set!)
          (arguments ((path (*state* batch invalid))
                      (value "not raw bytes") (expression? #f))))))))
    :expect (lambda (result)
              (and (list? result) (eq? (car result) 'error))))
  (test-submit
    ((*journal* journal-1 'get) '(*state* batch rolled-back))
    :expect '(nothing))
  ...
  (test-submit ((*journal* journal-1 'resolve)
     '(-1 journal-2 *state* a b) :pinned? #f :proof? #f) :expect '(directory ((c value)) #t))
  ...
  (let ((path '(6 journal-2 *state* a b c)))
  ...
  ;; Interface-secret rotation immediately replaces local credentials as well
  ;; as preparing a new committed Interface signing key.
  ...
  ;; Cross-user reads require an explicit authorization rule.
