(lambda (make-interface-harness)
  ...
  (with-let (make-interface-harness :journals 2 :users '(alice bob))
    ...
    (let* ((resolved
            (collect
              ((alice journal-1 journal-2 'resolve :history '(3 1))
               '(-1 *state* bob document) :pinned? #f :proof? #t)
              '(2 1 1 0)))
           (proof (cadr (assoc 'proof resolved)))
           (path '(3 journal-2 1 *state* bob document)))
      ...
      (test-submit ((*journal* journal-1 'pin!) path :response proof) :tick 1 :expect #t)
      (test-report)
      (test-submit
        ((*journal* journal-1 'resolve) path :pinned? #t :proof? #f)
        :expect '((content "version-1") (pinned? #t)))
      (test-submit ((*journal* journal-1 'unpin!) path) :expect #t)
      (test-submit
        ((*journal* journal-1 'resolve) path :pinned? #t :proof? #f)
  ...
  (with-let (make-interface-harness :journals 3 :journal-start 4 :users '(alice bob))
    ...
    (let* ((resolved
            (collect
              ...
              '(2 1 2 0 1 0)))
           (proof (cadr (assoc 'proof resolved)))
           (path '(2 journal-5 2 journal-6 0
                    *state* bob document)))
      ...
      (test-submit ((*journal* journal-4 'pin!) path :response proof) :tick 1 :expect #t)
      (test-report)
      (test-submit
        ((*journal* journal-4 'resolve) path :pinned? #t :proof? #f)
        :expect '((content "multi-v1") (pinned? #t)))
      (test-submit ((*journal* journal-4 'unpin!) path) :expect #t)
      (test-submit
        ((*journal* journal-4 'resolve) path :pinned? #t :proof? #f)
