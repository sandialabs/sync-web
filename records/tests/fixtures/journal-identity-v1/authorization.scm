(define-class (authorization)
  ...
  (define-method (~normalize-rule self rule latest-index)
    ...
    (let ((allowed '(principal key-index path get set! use! call! resolve)))
    ...
    (let* ((principal ((self '~normalize-principal)
           ...
           (call! (if (assoc 'call! rule) (cadr (assoc 'call! rule)) #f))
      ...
      (if (and (equal? principal '(*public*)) call!)
          (error 'authorization-error "Public principals cannot receive call! authority"))
      ...
      (for-each (lambda (entry)
                ...
                `((get ,get) (set! ,set!) (use! ,use!) (call! ,call!)))
             ...
             (or (let* ((rule (car rules))
                   ...
                   (and (or (equal? principal rule-principal)
                        ...
                        (case permission
                          ...
                          ((use!) (cadr (assoc 'use! rule)))
                          ((call!)
                           (and (not (equal? rule-principal '(*public*)))
                                (cadr (assoc 'call! rule))))
