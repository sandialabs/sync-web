...
- Interface code is responsible for authentication, authorization, stored-program execution, batching, root-object persistence, and orchestration of network fetches.
- `put!` and `put-batch!` accept inert or object values and optional expectations. Omitted expectations preserve unconditional writes; supplied expectations are checked together against one pre-write snapshot, return `#f` without writing on mismatch, and require destination `put!` plus read-only `use!` authority. Duplicate batch paths are compared against that same snapshot and successful replacements retain input order.
- `copy!` and `copy-batch!` capture raw Tree sources and optional target expectations from one pre-mutation staged snapshot. Missing sources delete targets, unavailable sources fail atomically, duplicate batch targets are last-write-wins in request order, and every source value remains the original snapshot value. They require read-only `use!` on each source, `put!` on each destination, and read-only `use!` on conditionally compared destinations.
- `truncate!` is a destructive Self-local operation for Root or configured Interface administrators. It applies one inclusive normalized cutoff atomically to permanent and temporary Ledger chains, releases locally available heads and pinned evidence through that cutoff, and preserves size, indexes, digests, staged state, retained suffixes, and future appends. Namespace ownership, Authorization grants, and federation cannot confer it.
- `prune!` and `prune-batch!` are Self-local Root/configured-administrator retention operations. They normalize canonical committed paths (including `-1`), reject the complete call if any target directly or ancestrally encompasses committed `*crypto*` material, prepare complete digest-preserving cuts for both temporary and permanent Ledger history, then install both or neither. Leaves and directories are idempotent; Stage, history identity, size, signatures, and numbering remain unchanged. Authorization grants and federation cannot confer pruning.
- `use-batch!` returns ordered staged results and takes one scalar `read-only?` mode. Mutating mode retains ordered successor visibility and full rollback; read-only mode evaluates in order and discards every successor. `put-batch!` compares one snapshot and applies ordered writes atomically. `retrieve-batch` accepts complete Self-relative committed paths, preserves order and duplicates across heterogeneous route/history groups, and keeps verified multiproofs internal.
- Linear and log Chains expose read-only `indices`, returning `(chain (<ascending materialized indexes>) <complete?>)`. Inventory never evaluates payloads; unavailable positions are omitted and make `complete?` false. Authenticated historical Chains without `indices` are scanned completely through their own embedded `size` and `get` inside `sync-let`, subject only to existing evaluator limits.
- `pin-batch!` prepares and verifies every remote proof group before one atomic local retention mutation. `unpin!` and `unpin-batch!` apply permanent-only digest-preserving cuts atomically after the same direct/ancestral committed-`*crypto*` protection used by pruning. Peer-facing `trace-batch` serializes the union of authorized same-head path accesses once.
- `run!` is implemented entirely by Interface: it loads the current staged Scheme procedure once for that dispatch, evaluates it outside `sync-let` in an Interface-owned masked environment, and applies it to an opaque authenticated journal capability followed by one explicit argument list.
- Invoking a staged program requires either root/configured-local-admin authority or an explicit recursive `run!` rule on the program path. Namespace ownership and read-only `use!` do not imply `run!`; every nested target and Journal operation preserves and rechecks the original caller.
- Stored programs call ordinary Interface methods as `((journal 'method) arguments ...)`; each call is an independent blocking Journal call, so `run!` does not promise atomicity.
- The journal capability fixes the original authentication context without exposing credentials, and Interface rejects any result containing procedures or other capability-bearing evaluator objects.
- `run!` is an ergonomic Scheme-native orchestration convenience, not a provenance boundary; durable provenance is expressed by the staged operations and committed transitions it invokes.
- `(*state* *periodic*)` is the one reserved periodic-program path. Only Root or a configured local Interface administrator may write/delete it; Authorization rules cannot grant that write.
- When a non-mutating outer Interface step completes a blocking Ledger commit that created a new index, it launches the current periodic program exactly once through detached `run!`, passing that zero-based index. Unchanged steps and internal step continuations never launch it. Launches from different indexes may overlap and complete out of order; no coalescing or single-flight semantics are implied. Dispatch is not a durable outbox: process failure after commit but before detached launch can lose that launch.
...
- `sync-let` freshly parses its body, exposes only the shared capability manifest through a fresh child of a closed request-local template, copies inert inputs/results, and rejects procedures or environments crossing the boundary. Its active-boundary and dynamic-evaluation machinery is internal and has no Scheme-visible helper primitive.
...
### Active Resource Leaves

- `put!` with `object? #t` accepts one `define-class` value and stores an uninitialized shell made by the Ledger's embedded Standard object. It never runs `*init*` during definition.
- `use!` receives a path, optional method, one explicit argument list, and scalar `read-only?` defaulting to `#f`. It supplies no hidden caller, route, path, time, random, network, Journal, or credential context.
- Mutating use persists one changed successor atomically. Read-only use returns the same validated result but always discards the successor and creates no transition.
- Blank inert read-only use is the staged byte/expression read. Named inert use is an error; blank object use returns `class`, `object-hash`, and `code-hash`. Object method `*api*` returns the raw embedded Standard `*api*` result through the same ordinary read-only `use!` or `retrieve` invocation boundary.
- `retrieve` and `retrieve-batch` authenticate committed/history selection, locally verify proof material and deterministic method results, and never persist successors. An explicit `$federation.route` may select one terminal responder for permanent-only retained retrieval: the responder interprets the committed path locally, applies ordinary `retrieve` authorization, rejects temp-only evidence, and never contacts the attributed source or another provider. A trailing alias selects its Chain inventory; continuing after an alias implies that Chain's `-1` head unless an exact integer follows. In terminal-provider mode, every explicit or implicit `-1` resolves recursively to the greatest materialized index in its containing permanent Chain, so a newer temp-only or unknown logical tail cannot shadow retained evidence. The terminal builds the response proof from the exact permanent object authenticated by the invocation. The origin anchors that returned proof digest, independently derives the same selections through the proof's authenticated embedded Chain code, and only then merges richer same-root route evidence to verify content at those exact paths.
- Symbols used as Tree-path names or bridge aliases must decode through the existing expression codec to one symbol with the same exact name and canonical re-encoding. Records rejects non-roundtrippable naming symbols before mutation, remote I/O, signing, or route lookup; it does not rewrite them or restrict unusual symbols in unrelated values.
- Current grants are `put!`, qualified `use!`, `run!`, and `retrieve`. `(use! ((read-only? #t)))` grants read-only use only; `#f` grants both modes.
...
- `standard.scm` provides shared operations for composing object methods across nested objects.
  ...
  - `deep-call!` evaluates its callback once and returns the proper list `(result successor-node)`, where the successor is the rebuilt root of the complete object argument.
