(define-class (federation)
  ...
  (define-method (*init* self standard (config '()))
    (let* ((secret (and (assoc 'secret config) (cadr (assoc 'secret config))))
           (key (and secret (crypto-generate
                             (expression->byte-vector `(interface ,secret))))))
      (if (not key) (error 'argument-error "Federation secret is required"))
      ((self '~field!) 'standard standard)
      ((self '~field!) 'config
       (expression->byte-vector
        (cons `(keys ((,(car key) ,(cdr key))))
              ((self '~alist-set) config 'secret #f #t))))))
  (define-method (config self (path '()))
    (cond ((equal? path '(interface-public-key))
           (caar ((self '~config-get) '(keys))))
          ((and (pair? path) (eq? (car path) 'keys))
           (error 'config-error "Private Federation keys are not public"))
  ...
  (define-method (update-config! self changes)
    ...
    (let ((path (cadr (assoc 'path changes)))
          (value (cadr (assoc 'value changes))))
      (case (car path)
        ((rotate-key)
         (let* ((keys ((self '~config-get) '(keys)))
                (key (crypto-generate
                      (expression->byte-vector
                       `(interface ,(cadr (assoc 'secret value))))))
                (committed (cadr (assoc 'committed-public-key value)))
                (old (and (byte-vector? committed) (assoc committed keys))))
           ((self '~config-set!) '(keys)
            (cons (list (car key) (cdr key))
                  (if (and old (not (equal? committed (car key)))) (list old) '())))
           (car key)))
        ((commit-key)
         (let ((configured (cadr (assoc 'configured value)))
               (committed (cadr (assoc 'committed value))))
           ((self '~config-set!) '(keys)
            (let loop ((keys ((self '~config-get) '(keys))) (selected '()))
              (if (null? keys) (reverse selected)
                  (loop (cdr keys)
                        (if (or (equal? (caar keys) configured)
                                (equal? (caar keys) committed))
                            (cons (car keys) selected) selected)))))))
        (else ((self '~config-set!) path value)))))
  ...
  (define-method (bridge! self ledger alias endpoint)
    ...
    (if operation-data
        ((self '~resume!) ledger 'bridge! alias
         `((alias ,alias) (endpoint ,endpoint)) operation-data)
        (begin
    ...
    (let* ((name (cadr (assoc 'name request)))
           ...
           (verified
            ((self '~verify-peer-response)
             (cadr (assoc 'response request)) info interface
             ledger-checkpoint))
           (result
  ...
  (define-method (bridge-synchronize! self ledger alias)
    ...
    (if operation-data
        ((self '~resume!) ledger 'bridge-synchronize!
         alias `((alias ,alias)) operation-data)
  ...
  (define-method (~resume! self ledger expected-operation alias expected-arguments operation-data)
    ...
    (if (not (and
              ...
              (equal? (cadr (assoc 'arguments operation-data))
                      expected-arguments)
              ((self '~continuation-verify?) operation-data)))
    ...
    (let ((ledger-matches?
          ...
          (federation-matches?
           (equal? (cadr (assoc 'expected-federation-peer operation-data))
                   ((self '~federation-peer-digest) alias))))
      (if (not (and ledger-matches? federation-matches?))
  ...
  (define-method (~crossed-bridge? self alias expected-arguments)
    (let* ((peer ((self 'peer) alias))
           (endpoint (cadr (assoc 'endpoint expected-arguments))))
      (and (not (null? peer))
           (eq? ((self '~ref) peer '(initiation)) 'remote)
           (equal? ((self '~ref) peer '(interface))
                   ((self '~ref) endpoint '(interface)))
           (equal? ((self '~ref) peer '(remote-name))
                   ((self '~ref) endpoint '(remote-name))))))

  (define-method (~continuation self body)
    (let* ((body
            (cons '(domain sync-web/federation-continuation/v1) body))
           (keys ((self '~config-get) '(keys)))
           (key (and (pair? keys) (car keys))))
      (if (not (and (list? key) (= (length key) 2)
                    (byte-vector? (car key)) (byte-vector? (cadr key))))
          (error 'authentication-error
                 "Federation has no continuation signing key"))
      (append body
              `((signature
                 ,(crypto-sign
                   (cadr key) (expression->byte-vector body)))))))

  (define-method (~continuation-verify? self operation-data)
    (let* ((body ((self '~alist-set) operation-data 'signature #f #t))
           (signature (cadr (assoc 'signature operation-data)))
           (keys ((self '~config-get) '(keys))))
      (and (byte-vector? signature)
           (let loop ((keys keys))
             (and (pair? keys)
                  (or (and (list? (car keys)) (byte-vector? (caar keys))
                           (crypto-verify
                            (caar keys) signature
                            (expression->byte-vector body)))
                      (loop (cdr keys))))))))
      ...
      (if (pair? route-target)
          ...
          (let* ((index (if (< index 0) (- ((ledger 'size)) 1) index))
            ...
            `((route-source ,route-source)
              ...
              (roots ,(reverse roots))
              (object ,((standard 'serialize) object #f))
              ,@(if history-object
  ...
  (define-method (invoke self ledger operation arguments route history identity)
    ;; Construct, sign, deliver, and verify one federated application request.
    (if (not (memq operation '(get set! resolve)))
    ...
    (let* ((standard (sync-eval ((self '~field!) 'standard)))
           ...
           (key-entry (and (byte-vector? source-key)
                           (assoc source-key ((self '~config-get) '(keys)))))
           (private-key (and key-entry (cadr key-entry)))
           ...
           (signature
            (crypto-sign
             (if private-key private-key
                 (error 'authentication-error
                        "No retained private key matches the terminal's committed object"))
             (expression->byte-vector
  ...
  (define-method (authenticate self ledger invocation)
    ...
    (let* ((standard (sync-eval ((self '~field!) 'standard)))
      ...
      (if (or (not (memq operation '(get set! resolve)))
              (not (symbol? identity))
          ...
          (error 'authentication-error "Invalid terminal invocation envelope"))
      (let* ((object ((ledger 'read) terminal-index
        ...
        (let ((data-head
          ...
          `((principal ,((self '~route-principal) route-source identity))
            ...
            ,@(if data-head `((data-head ,data-head)) '()))))))
