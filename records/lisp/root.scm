(macro (secret clear?)
  (define transition-function
    '(lambda (*sync-state* query)
       (let* ((transition-node (sync-car *sync-state*))
         ...
         (cons (case (car query)
                 ...
                 ((*call*) (apply root-call (cdr query)))
                 ((*step*) (apply root-step (cdr query)))
                 ((*set-secret*) (apply root-set (append '(secret) (cdr query))))
                 ((*set-step*) (apply root-set (append '(step) (cdr query))))
                 ((*set-query*) (apply root-set (append '(query) (cdr query))))
                 (else (root-query query)))
               *sync-state*))))
