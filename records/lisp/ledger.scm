(define-class (ledger)
  ...
  (define-method (set! self path value)
    ...
    (let ((stage-obj (sync-eval ((self '~field!) 'stage) #f)))
      ...
      ((stage-obj 'set!) '(*transition* operation) `((path ,path) (value ,value)))
      (if (> (length path) 2)
          (let ((standard (sync-eval ((self '~field!) 'standard) #f)))
            ((self '~field!) 'stage ((standard 'deep-set!) (stage-obj) path value)))
          (begin ((stage-obj 'set!) (car path) value)
                 ((self '~field!) 'stage (stage-obj))))))

  (define-method (set-batch! self paths values)
    (let ((stage-obj (sync-eval ((self '~field!) 'stage) #f)))
      (let loop ((paths paths) (values values))
        (if (and (null? paths) (null? values)) ((self '~field!) 'stage (stage-obj))
            (begin ((self '~path-check) (car paths) #t #t)
                   ((stage-obj 'copy!) '(*transition*) '(*transition* previous))
                   ((stage-obj 'set!) '(*transition* operation) `((path ,(car paths)) (value ,(car values))))
                   ((stage-obj 'set!) (caar paths) (car values))
                   (loop (cdr paths) (cdr values)))))))
  ...
  (define-method (bridge-synchronize! self name index response)
    ...
    (let* ((standard (sync-eval ((self '~field!) 'standard) #f))
           ...
           (custom-set! (lambda (path value)
                        ((stage 'copy!) '(*transition*) '(*transition* previous))
                        ((stage 'set!) '(*transition* operation) `((path ,path) (value ,value)))
                        ((stage 'set!) (car path) value))))
          ...
          (let* ((init (> ((perm 'size)) 0))
            ...
            (if (and last (> index 0) (< index (- ((last 'size)) 1))) #f
                (let* ((deserialized ((standard 'deserialize) response))
                  ...
                  (if (< index 0)
                      (custom-set! `((*bridge* ,name valid?)) #t)
