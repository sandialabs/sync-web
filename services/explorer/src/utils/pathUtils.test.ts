...
describe('generateExpandedNodesFromPath', () => {
  it('should generate expanded nodes for a simple path', () => {
    ...
    expect(result.has(JSON.stringify(['*state*']))).toBe(true);
    expect(result.has(JSON.stringify(['*state*', 'dir1']))).toBe(true);
  ...
  it('should generate expanded nodes for a path with version', () => {
    ...
    expect(result.has(JSON.stringify([-1, '*state*']))).toBe(true);
