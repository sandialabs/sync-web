...
mod tests {
    ...
    fn unified_explicit_quote_splicing_matches_s7() {
        ...
        let cases = [
            ...
            ("(eval '(let ((x 7)) `(quote (a ,x b))))", "(quote (a 7 b))"),
            ("(eval '(let ((x '(1 2))) `(quote (a ,@x b))))", "(quote (a 1 2 b))"),
            ...
            ("(eval '(let ((x '(1 2))) `#(',@x)))", "#(',(apply-values x))"),
            ("(eval '(let ((x '(1 2))) `#('(a ,@x b))))", "#('(a ,(apply-values x) b))"),
            ("(eval '(let ((x '(1 2))) `#((outer '(a ,@x b)))))", "#((outer '(a ,(apply-values x) b)))"),
            ("(eval '(let ((x 7)) `#('(a ,x b))))", "#('(a ,x b))"),
            ("(eval '(let ((x '(1 2))) `#('(a ,@x b) (quote (c ,@x d)))))", "#('(a ,(apply-values x) b) (quote (c ,(apply-values x) d)))"),
            ("(eval '(let ((x '(1 2)) (y '(3 4))) `#('(a ,@x b ,@y c))))", "#('(a ,(apply-values x) b ,(apply-values y) c))"),
            ("(eval '(let ((n 0) (x '(1 2))) (list `#('(a ,@(begin (set! n (+ n 1)) x) b)) n)))", "(#('(a ,(apply-values (begin (set! n (+ n 1)) x)) b)) 0)"),
            ("(equal? (expression->byte-vector (eval '(let ((x '(1 2))) `#('(a ,@x b))))) (expression->byte-vector '#('(a ,(apply-values x) b))))", "#t"),
            ("(eval '(let ((x 7)) `#('(outer `(inner ,,x)))))", "#('(outer `(inner ,,x)))"),
            ("(eval '(let ((x '(1 2))) `#('(outer `(inner ,,@x)))))", "#('(outer `(inner ,,@x)))"),
            ("(eval '(let ((x 7)) `#((quote (outer `(inner ,,x))))))", "#('(outer `(inner ,,x)))"),
            ("(eval '(let ((x '(1 2))) `#((quote (outer `(inner ,,@x))))))", "#('(outer `(inner ,,@x)))"),
            ("(eval '(let ((n 0) (x '(1 2))) (list `#('(outer `(inner ,,(begin (set! n (+ n 1)) x)))) n)))", "(#('(outer `(inner ,,(begin (set! n (+ n 1)) x)))) 0)"),
            ("(equal? (expression->byte-vector (eval '(let ((x '(1 2))) `#('(outer `(inner ,,@x)))))) (expression->byte-vector '#('(outer `(inner ,,@x)))))", "#t"),
            ("(equal? (expression->byte-vector (eval '(let ((x '(1 2))) `#((quote (outer `(inner ,,@x))))))) (expression->byte-vector '#('(outer `(inner ,,@x)))))", "#t"),
            ...
            ("(eval '(let* ((p (list 'a)) (x (list p)) (r `(quote ,@x))) (set-car! p 'z) (list (eq? (cadr r) p) r)))", "(#t (quote (z)))"),
            ("(eval '(let* ((x (list 1 2)) (y (list 3 4)) (r `(quote a ,@x b ,@y c))) (set-car! x 9) (set-car! y 8) r))", "(quote a 1 2 b 3 4 c)"),
            ...
            ("(eval '(let ((x (list 1 2))) (set-cdr! (cdr x) x) (catch #t (lambda () `(quote ,@x)) (lambda args args))))", "(wrong-type-arg (\"apply's last argument should be a proper list: ~S\" (#1=(1 2 . #1#))))"),
    ...
    fn native_codecs_round_trip_scheme_and_hex() {
        ...
        assert_eq!(
            host.evaluate_unified_output("(list (let* ((v #u(1 2 3 4)) (s (subvector v 1 3))) (reverse! v) (list v s)) (let* ((v #u(1 2 3 4)) (s (subvector v 1 3))) (reverse! s) (list v s)) (let* ((v #u(3 1 2 4)) (s (subvector v 0 3))) (sort! v <) (list v s)) (let* ((v #u(3 1 2 4)) (s (subvector v 0 3))) (sort! s <) (list v s)))"),
            Ok("((#u(4 3 2 1) #u(3 2)) (#u(1 3 2 4) #u(3 2)) (#u(1 2 3 4) #u(1 2 3)) (#u(1 2 3 4) #u(1 2 3)))".into())
        );
        assert_eq!(
            host.evaluate_unified_output("(list (let* ((n 0) (v #u(3 1 2 4)) (s (subvector v 0 3))) (list (catch #t (lambda () (sort! s (lambda (a b) (set! n (+ n 1)) (if (= n 1) (error 'stop \"stop\") (< a b))))) (lambda args 'error)) v s n)) (let* ((n 0) (v #u(3 1 2 4)) (s (subvector v 0 3))) (sort! s (lambda (a b) (set! n (+ n 1)) (< a b))) (list v s n)))"),
            Ok("((error #u(3 1 2 4) #u(3 1 2) 1) (#u(1 2 3 4) #u(1 2 3) 3))".into())
        );
        assert_eq!(
            host.evaluate_unified_output("(list (let* ((v #(3 1 2 4)) (s (subvector v 0 3))) (reverse! s) (sort! v <) (set! (s 0) 8) (fill! s 7 1 2) (copy #(6) s 0) (list v s)) (let* ((v #i(3 1 2 4)) (s (subvector v 0 3))) (reverse! s) (sort! v <) (set! (s 0) 8) (fill! s 7 1 2) (copy #i(6) s 0) (list v s)) (let* ((v #r(3 1 2 4)) (s (subvector v 0 3))) (reverse! s) (sort! v <) (float-vector-set! s 0 8) (fill! s 7 1 2) (copy #r(6) s 0) (list v s)) (let* ((v #u(3 1 2 4)) (s (subvector v 0 3))) (reverse! s) (sort! v <) (set! (s 0) 8) (fill! s 7 1 2) (copy #u(6) s 0) (list v s)))"),
            Ok("((#(6 7 3 4) #(6 7 3)) (#i(6 7 3 4) #i(6 7 3)) (#r(6.0 7.0 3.0 4.0) #r(6.0 7.0 3.0)) (#u(6 7 3 4) #u(6 7 3)))".into())
        );
        assert_eq!(
            host.evaluate_unified_output("(list (let* ((n 0) (v #u(3 1 2 4)) (s (subvector v 0 3))) (sort! s (lambda (a b) (set! n (+ n 1)) (if (= n 1) (set! (v 3) 9)) (< a b))) (list v s n)) (let* ((n 0) (v #u(3 1 2 4)) (s (subvector v 0 3))) (sort! s (lambda (a b) (set! n (+ n 1)) (if (= n 1) (set! (s 2) 8)) (< a b))) (list v s n)))"),
            Ok("((#u(1 2 3 9) #u(1 2 3) 3) (#u(1 2 3 4) #u(1 2 3) 3))".into())
        );
