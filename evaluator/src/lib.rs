...
impl Evaluator {
    ...
    fn materialize_bytecode_env(&self,bc:&BytecodeFunction,env:&EnvRef,base_slots:&[Value],temps:&[Value])->EnvRef{let fb=Env::new(Some(env.clone()));if let crate::bytecode::BytecodeLayout::SlotFrame{names}=&bc.layout{for (name,value) in names.iter().zip(base_slots.iter()){fb.define(name.as_str(),value.clone());}}for instr in &bc.code{if let Instr::BindTemp{index,name,..}=instr{if let Some(value)=index.checked_sub(base_slots.len()).and_then(|j|temps.get(j)){fb.define(name.as_str(),value.clone());}}}fb}
    #[inline(never)]
    ...
    fn eval_bytecode_body(&mut self, bc:&BytecodeFunction, env:EnvRef, slots:Option<&[Value]>)->Result<Value>{
        ...
        loop{
            ...
            match instr{
                ...
                Instr::Fallback(i)=>{let expr=bc.constants.get(*i).cloned().ok_or_else(||SchemeError::new("unsupported-compiled-form",vec![]))?;let fb=self.materialize_bytecode_env(bc,&env,base_slots,&temps);stack.push(self.eval(expr,fb)?);}
                Instr::MakeLambda(i)=>{let l=bc.lambdas.get(*i).ok_or_else(||SchemeError::new("unsupported-compiled-form",vec![]))?;let fb=self.materialize_bytecode_env(bc,&env,base_slots,&temps);stack.push(self.compiled_lambda_value(l,fb));}
                Instr::ApplyLambda(i)=>{let tail=stack.pop().ok_or_else(||SchemeError::new("unsupported-compiled-form",vec![]))?;let l=bc.lambdas.get(*i).ok_or_else(||SchemeError::new("unsupported-compiled-form",vec![]))?;if let (Ok(args),Some(lambda_bc))=(tail.to_vec(),l.bytecode.as_ref()){if args.len()==l.params.len(){let fb=self.materialize_bytecode_env(bc,&env,base_slots,&temps);match self.eval_bytecode_body(lambda_bc,fb,Some(&args)){Ok(v)=>{stack.push(v);continue},Err(e) if e.tag=="unsupported-compiled-form"=>{},Err(e)=>return Err(e)}}}let fb=self.materialize_bytecode_env(bc,&env,base_slots,&temps);let proc=self.compiled_lambda_value(l,fb);let Some((func,_,_))=env.builtin_func("apply") else{return Err(SchemeError::new("unsupported-compiled-form",vec![]));};stack.push(func(self,&[proc,tail])?);}
