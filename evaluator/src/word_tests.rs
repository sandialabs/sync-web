...
mod tests{
    ...
    fn resumable_word_frame_continues_after_host_call_without_replay(){
        fn make_machine()->ResumableWordMachine{let function=BytecodeFunction{layout:BytecodeLayout::DynamicEnv,constants:vec![Value::Int(0),Value::Int(2)],code:vec![
            ...
            Instr::IntAddTermsRecur{terms:vec![AddTerm::Slot(0),AddTerm::Const(1)]},Instr::LoadTemp(1),Instr::Recur{argc:2,target:4,param_start:0,param_count:2,name:std::rc::Rc::new("loop".into())},Instr::Return],max_temps:2,cache_stable_builtins:true,required_builtins:vec!["+"],lambdas:vec![],materialization_bindings:vec![],validated_env:Cell::new((0,0)),word_program:None,word_calls_safe:true};let program=std::rc::Rc::new(WordProgram::compile(&function).unwrap());let env=Env::new(None);env.define("host",Value::string("host"));ResumableWordMachine::new(program,&[],&env).unwrap()}
    ...
    fn rooted_frame_stack_resumes_nested_host_mutation_once(){
        fn function(constants:Vec<Value>,code:Vec<Instr>)->BytecodeFunction{BytecodeFunction{layout:BytecodeLayout::DynamicEnv,constants,code,max_temps:0,cache_stable_builtins:true,required_builtins:vec![],lambdas:vec![],materialization_bindings:vec![],validated_env:Cell::new((0,0)),word_program:None,word_calls_safe:true}}
    ...
    fn fixed_word_bytecode_executes_complete_integer_loop(){
        let function=BytecodeFunction{layout:BytecodeLayout::DynamicEnv,constants:vec![Value::Int(0)],code:vec![
            ...
            Instr::LoadTemp(0),Instr::Return],max_temps:1,cache_stable_builtins:true,required_builtins:vec![],lambdas:vec![],materialization_bindings:vec![],validated_env:Cell::new((0,0)),word_program:None,word_calls_safe:true};
