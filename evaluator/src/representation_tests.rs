...
mod tests{
    ...
    #[test]
    fn bytecode_let_star_initializers_use_only_active_slots(){
        let source="((lambda () (let* ((a 0) (x (let ((f (lambda (key) (if (eq? key 'size) (lambda () 7) #f)))) f)) (y 1)) ((x 'size)))))";
        assert_eq!(run_source_output(source).unwrap(),"7");
    }

    #[test]
    fn bytecode_closure_captures_only_active_lexical_bindings(){
        let future="((lambda (x) (let ((f (lambda () x))) (let ((x 'wrong)) #f) (f))) 'right)";
        let expired="((lambda (x) (let ((f #f)) (let ((x 'wrong)) (set! f (lambda () x))) (let ((x 'later)) #f) (f))) 'right)";
        assert_eq!(run_source_output(future).unwrap(),"right");
        assert_eq!(run_source_output(expired).unwrap(),"wrong");
    }

    #[test]
    fn bytecode_nested_initializer_effects_execute_once(){
        let source="(let ((count 0)) (list ((lambda () (let* ((a 0) (x (let ((f (begin (set! count (+ count 1)) (lambda () 7)))) f)) (y 1)) (x)))) count))";
        assert_eq!(run_source_output(source).unwrap(),"(7 1)");
    }

    #[test]
    fn nested_quasiquote_preserves_runtime_unquote_splicing(){
        let representation="(let ((x '(a b))) `(outer `(inner ,@x)))";
        let generated="(let ((init '(define-method (make self class) (list class)))) (let ((form `(lambda () ((eval `(lambda* ,(cddadr init) ,@(cddr init))) '(define-class standard))))) ((eval form))))";
        let recode="(let ((function `(lambda () (let ((recode (lambda (code) `(lambda (obj) (list ,code obj))))) ((eval (recode #u(1 2))) 'payload))))) ((eval function)))";
        assert_eq!(run_source_output(representation).unwrap(),"(outer (list-values 'inner (apply-values x)))");
        assert_eq!(run_source_output(generated).unwrap(),"((define-class standard))");
        assert_eq!(run_source_output(recode).unwrap(),"(#u(1 2) payload)");
    }
