...
pub(crate) struct PairCell{pub(crate) data:RefCell<PairData>}
...
#[derive(Clone)]pub struct NumberLiteralData{pub(crate) repr:Rc<String>,pub(crate) value:f64}
...
impl Env {
    pub(crate) fn new(parent: Option<EnvRef>) -> EnvRef {
        let guard_epoch=parent.as_ref().map(|p|p.guard_epoch.clone()).unwrap_or_else(||Rc::new(Cell::new(0)));let callable_shadowed=parent.as_ref().map(|p|p.callable_shadowed.clone()).unwrap_or_else(||Rc::new(Cell::new(false)));
        Rc::new(Self { parent: RefCell::new(parent), vars: RefCell::new(EnvVars::new()), order: RefCell::new(Vec::new()), open: RefCell::new(false), guard_epoch, callable_shadowed })
    }
    pub(crate) fn with_capacity(parent: Option<EnvRef>, cap: usize) -> EnvRef {
        let guard_epoch=parent.as_ref().map(|p|p.guard_epoch.clone()).unwrap_or_else(||Rc::new(Cell::new(0)));let callable_shadowed=parent.as_ref().map(|p|p.callable_shadowed.clone()).unwrap_or_else(||Rc::new(Cell::new(false)));
        Rc::new(Self { parent: RefCell::new(parent), vars: RefCell::new(EnvVars::with_capacity(cap)), order: RefCell::new(Vec::with_capacity(cap)), open: RefCell::new(false), guard_epoch, callable_shadowed })
    }
...
impl Value {
    ...
    pub(crate) fn number_literal(repr:Rc<String>,value:f64)->Self{Value::NumberLiteral(Rc::new(NumberLiteralData{repr,value}),())}
...
fn push_quote_symbol_shorthand(out:&mut String,value:&Value)->bool{let Value::Pair(pair)=value else{return false};let pair=pair.borrow();let Some(head)=pair.car.as_symbol() else{return false};let Value::Pair(arguments)=&pair.cdr else{return false};let arguments=arguments.borrow();match (head,&arguments.car){("quote"|"quasiquote",Value::Symbol(symbol))=>{out.push('\'');out.push_str(symbol);true},("unquote",argument)=>{out.push(',');out.push_str(&argument.to_string());true},_=>false}}
...
fn write_value_direct_mode(out:&mut String,v:&Value,acyclic:bool){
    ...
    fn rec(out:&mut String,v:&Value,seen:&mut DirectSeen,acyclic:bool){
        match v{
            ...
            Value::Int(n)=>push_i64_s7(out,*n),Value::RationalValue(r)=>{push_i64_s7(out,r.num);out.push('/');push_i64_s7(out,r.den);},Value::Float(x)=>push_float_s7(out,*x),Value::ComplexValue(c)=>{push_float_s7(out,c.real);if c.imag>=0.0{out.push('+');}push_float_s7(out,c.imag);out.push('i');},Value::NumberLiteral(s,_)=>out.push_str(&s.repr),
...
fn fmt_value(f: &mut fmt::Formatter<'_>, v: &Value, seen: &mut HashSet<usize>) -> fmt::Result {
    match v {
        ...
        Value::Int(n)=>write!(f,"{}",n), Value::RationalValue(r)=>write!(f,"{}/{}",r.num,r.den), Value::Float(x)=>fmt_float_num(f,*x), Value::ComplexValue(c)=>{fmt_float_num(f,c.real)?;if c.imag>=0.0{write!(f,"+")?;}fmt_float_num(f,c.imag)?;write!(f,"i")}, Value::NumberLiteral(s,_)=>write!(f,"{}",s.repr),
