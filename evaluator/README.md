...
The Rust port should target sync-web-compatible s7 behavior, not necessarily full upstream s7 embedding compatibility. The current runtime representation, execution tiers, guard rules, and safety invariants are summarized in [`docs/runtime-architecture.md`](docs/runtime-architecture.md). Private unified-runtime ownership, rooting, resumability, graph, and request-boundary invariants are frozen in [`docs/unified-runtime-readiness.md`](docs/unified-runtime-readiness.md).
